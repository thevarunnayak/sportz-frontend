  ...
  const handleWSMessage = useCallback((msg: WSMessage) => {
    switch (msg.type) {
      case "score_update":
        ...
        break;
      case "commentary": {
        // Some servers may include the matchId at the top level (msg.matchId) or inside the data payload.
        // Ensure we handle both shapes so real-time updates always render.
        const incomingMatchId = (msg as any).matchId ?? msg.data.matchId;
        if (
          latestMatchIdRef.current == null ||
          incomingMatchId != latestMatchIdRef.current
        ) {
          return;
        }
    ...
    try {
      ...
      setMatches((prevMatches) => {
        ...
        return nextMatches.map((match) => {
          ...
          const prev = prevById.get(matchId) as Match | undefined;
